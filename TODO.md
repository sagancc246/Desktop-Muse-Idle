...
- [ ] `src/effects/` を作成する
- [ ] `src/effects/effectTypes.ts` を作成する
- [ ] `src/effects/effectManager.ts` を作成する
- [ ] `src/effects/particleFactory.ts` を作成する
- [ ] `src/effects/floatingText.ts` を作成する
- [ ] `src/effects/screenFlash.ts` を作成する
- [ ] `src/effects/ringEffect.ts` を作成する
...
- [ ] 初回プレイ用の短いチュートリアルを追加する
- [ ] Corner Hit / Muse Tap / Galleryの基本操作を案内する
- [ ] 設定画面とGalleryをキーボードで操作できるようにする
- [ ] `Enter` / `Esc` の決定・戻る操作を整理する
- [ ] ボタンや音量スライダーにアクセシブルなラベルを付与する

### 統計・蓄積表示

- [ ] 統計画面を追加する
- [ ] 総壁ヒット数 / 総Corner Hit数を表示する
- [ ] 最高到達Stage / Reboot回数を表示する
- [ ] プレイ時間 / 獲得背景数を表示する
